...
// --- Orders (order_group) ---
// 1: single buyer, SO still open (live 25-min countdown demo)
// 2: single buyer, fully verified & stamped
// 3: single buyer, still draft (no SO yet)
...
insDeal.run(1, 1, 'EX', '2026-08-10', '2026-08-14', 'advance', 70, 30, '', 'so_generated');       // deal 1 -> order1, Om Traders
...
insDeal.run(4, 1, 'EX', '2026-08-09', '2026-08-11', 'advance', 30, 70, '', 'so_generated');        // deal 5 -> order4 leg B, Om Traders (same order, different buyer)
...
// --- Sale Orders ---
// so1 -> deal1: freshly generated, live 25-min countdown (demo of hard expiry)
// so2 -> deal2: verified long ago
// so3 -> deal4: verified long ago
// so4 -> deal5: generated long ago, past its 25-min window, never responded -> EXPIRED
// so5 -> deal6: verified, later disputed
const insSO = db.prepare(`INSERT INTO sale_order (deal_id, transporter_id, sequential_code, link_hash, status, resend_count, generated_at, expires_at, responded_at, frozen_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);

insSO.run(1, null, '1000234', 'a1f5c9', 'sent', 0, iso(now), minutesFromNow(25), null, null);
insSO.run(2, 1, '1000235', 'b2e6d0', 'verified', 0, daysAgo(6), daysAgoPlusMin(6, 25), daysAgoPlusMin(6, 12), daysAgoPlusMin(6, 12));
insSO.run(4, 1, '1000236', 'c3d7e1', 'verified', 1, daysAgo(8), daysAgoPlusMin(8, 25), daysAgoPlusMin(8, 20), daysAgoPlusMin(8, 20));
insSO.run(5, null, '1000238', 'e5b9a3', 'expired', 0, daysAgo(3), daysAgoPlusMin(3, 25), null, null);
insSO.run(6, 3, '1000237', 'd4c8f2', 'verified', 0, daysAgo(18), daysAgoPlusMin(18, 25), daysAgoPlusMin(18, 15), daysAgoPlusMin(18, 15));

// --- Invoice Ack Recs (only for verified SOs: so2, so3, so5) ---
const insAck = db.prepare(`INSERT INTO invoice_ack_rec
  (so_id, parent_id, invoice_number, invoice_date, ocr_verified, status, otp_code, otp_attempts, otp_locked, otp_verified_at, estamp_ref, estamp_timestamp, grace_period_ends_at, distributed_at, distributed_channels)
...
console.log('Live demo row: SO #1 (order ORD-000001, Om Traders) expires 25 min from seed time — good for testing the hard-expiry countdown.');
db.close();
